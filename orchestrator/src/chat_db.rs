...
impl ChatDb {
    ...
    fn migrate(&self) -> Result<()> {
        ...
        // Migration: add lesson_id to conversations if not exists (schema v8)
        let has_lesson_id: i64 = conn.query_row(
            "SELECT COUNT(*) FROM pragma_table_info('conversations') WHERE name = 'lesson_id'",
            [],
            |row| row.get(0),
        )?;
        if has_lesson_id == 0 {
            conn.execute(
                "ALTER TABLE conversations ADD COLUMN lesson_id TEXT REFERENCES lessons(id)",
                [],
            )?;
            conn.execute(
        ...
        if has_courses == 0 {
            ...
            tracing::info!("Migrated schema v9: added courses, modules, lessons, enrollments, progress tables");
        }
...
pub fn add_message(
    ...
    pub fn update_course_lesson_progress(&self, enrollment_id: &str, lesson_id: &str, status: &str) -> Result<LessonProgress> {
        let conn = self.conn.lock().unwrap();
        let id = uuid::Uuid::new_v4().to_string();
